...
LUA_SHA1 = get_hash(os.path.join(CWD, 'pinger.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'm':
                wget_args.extend(['--warc-header', 'pinger-m: '+item_value])
                wget_args.append('http://dummy.pinger.pl/m/'+item_value)
            elif item_type == 'profile':
                wget_args.extend(['--warc-header', 'pinger-profile: '+item_value])
                wget_args.append('http://'+item_value+'.pinger.pl/')
            else:
...
project = Project(
    title='pinger',
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/9/90/Pinger-logo.png" height="50px" title=""/>
        <h2>pinger.pl <span class="links"><a href="https://pinger.pl/">Website</a> &middot; <a href="http://tracker.archiveteam.org/pinger/">Leaderboard</a></span></h2>
        <p>Archiving everything from pinger.</p>
    '''
)
